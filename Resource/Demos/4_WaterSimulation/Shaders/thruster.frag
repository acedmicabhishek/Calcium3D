...
void main()
{
    ...
    float verticalMask = 1.0 - smoothstep(0.0, 1.0, TexCoords.y);
    ...
    float turb = hash(TexCoords.y * 10.0 + TexCoords.x * 5.0 - time * 20.0) * 0.1;
    float noiseGlow = horizGlow + turb * verticalMask;

    float coreFactor = pow(clamp(noiseGlow, 0.0, 1.0), 6.0) * (1.0 - TexCoords.y * 1.5);
