...
bool AudioEngine::RayIntersectsAABB(const glm::vec3& rayOrigin, const glm::vec3& rayDir,
...
float AudioEngine::ComputeOcclusion(Scene* scene, const glm::vec3& sourcePos, const glm::vec3& listenerPos, int sourceIndex) {
    if (glm::length(listenerPos - sourcePos) < 0.001f) return 0.0f;
    ...
    for (int r = 0; r < 5; r++) {
        ...
        for (int i = 0; i < (int)objects.size(); i++) {
            ...
            if (!objects[i].acousticMaterial.isAcousticObstacle) continue;
            if (objects[i].meshType == MeshType::None) continue;
            ...
            rotMat[0] /= globalScale.x; rotMat[1] /= globalScale.y; rotMat[2] /= globalScale.z;
            ...
            float tHit;
            if (RayIntersectsAABB(sourcePos, dir, worldAABB.min, worldAABB.max, tHit)) {
                if (tHit < totalDist) {
...
float AudioEngine::ComputeReverb(Scene* scene, const glm::vec3& sourcePos, float& outEchoDelay, float& outEchoDecay) {
    
    static const glm::vec3 rayDirs[] = {
    ...
    for (int r = 0; r < numRays; r++) {
        ...
        for (int i = 0; i < (int)objects.size(); i++) {
            ...
            AABB worldAABB = PhysicsEngine::GetTransformedAABB(
                objects[i].collider, objects[i].position, objects[i].rotation, objects[i].scale
            );
            
            float tHit;
            if (RayIntersectsAABB(sourcePos, rayDirs[r], worldAABB.min, worldAABB.max, tHit)) {
...
float AudioEngine::ComputeDopplerPitch(const glm::vec3& sourcePos, const glm::vec3& prevSourcePos,
                                        const glm::vec3& listenerPos, const glm::vec3& prevListenerPos,
                                        float deltaTime, float dopplerFactor) {
    if (deltaTime <= 0.0001f) return 1.0f;
...
void AudioEngine::Update(Scene* scene, const glm::vec3& listenerPos, const glm::vec3& listenerDir, const glm::vec3& listenerUp, float deltaTime) {
    if (!s_Initialized || !scene) return;

    
    ma_engine_listener_set_position(s_Engine, 0, listenerPos.x, listenerPos.y, listenerPos.z);
    ma_engine_listener_set_direction(s_Engine, 0, listenerDir.x, listenerDir.y, listenerDir.z);
    ma_engine_listener_set_world_up(s_Engine, 0, listenerUp.x, listenerUp.y, listenerUp.z);
    ...
    for (int idx = 0; idx < (int)objects.size(); idx++) {
        ...
        if (obj.audio.pSource) {
            ...
            if (obj.audio.enableDoppler && obj.audio.type == AudioType::Directional) {
                dopplerPitch = ComputeDopplerPitch(
                    worldPos, obj.prevPosition,
            ...
            if (obj.audio.enableOcclusion && obj.audio.type == AudioType::Directional) {
                obj.audio.occlusionFactor = ComputeOcclusion(scene, worldPos, listenerPos, idx);
                
                
                
                occlusionVolMult = std::max(0.0f, 1.0f - obj.audio.occlusionFactor);
                
                occlusionPitchMult = std::max(0.5f, 1.0f - (obj.audio.occlusionFactor * 0.5f)); 
            }
            
            
            if (obj.audio.enableReverb && obj.audio.type == AudioType::Directional) {
                float echoDelay, echoDecay;
                obj.audio.reverbMix = ComputeReverb(scene, worldPos, echoDelay, echoDecay);
...
void AudioEngine::PlayObjectAudio(GameObject& obj) {
    if (!s_Initialized) return;
    if (obj.audio.filePath.empty()) return;
    ...
    ma_result result = ma_sound_init_from_file(s_Engine, obj.audio.filePath.c_str(), 0, NULL, NULL, sound);
    if (result != MA_SUCCESS) {
        Logger::AddLog("[AudioEngine] Failed to load sound: %s", obj.audio.filePath.c_str());
        delete sound;
        return;
    }
    ...
    if (obj.audio.type == AudioType::Ambience) {
        ...
        ma_sound_set_position(sound, obj.position.x, obj.position.y, obj.position.z);
        ma_sound_set_min_distance(sound, obj.audio.minDistance);
        ma_sound_set_max_distance(sound, obj.audio.maxDistance);
