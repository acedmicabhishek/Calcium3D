...
void Console::ExecuteCommand(const std::string &cmd) {
  ...
  if (parsed == "help") {
    ...
    AddLog(
        "  /settingsRendering  — Rendering settings (wireframe, MSAA, VSync)");
    AddLog("  /settingsPhysics    — Physics engine configuration");
    AddLog(
        "  /settingsEnv        — Environment settings (sun, moon, time, sky)");
    ...
    if (m_EngineLoggingEnabled) {
      AddLog("  Current Target App State: %s",
             StateManager::GetCurrentStateName().c_str());
    ...
    if (arg == "true" || arg == "on") {
      ...
      if (m_ActivePanel == SettingsPanel::Logging)
    ...
    AddLog("  FPS: %.1f (%.3f ms/frame)", ImGui::GetIO().Framerate,
    ...
    std::string arg = (parsed.size() > 7) ? parsed.substr(7) : "";
    if (arg == "on")
      HitboxGraphics::ShowHitboxes = true;
    else if (arg == "off")
      HitboxGraphics::ShowHitboxes = false;
    else
      HitboxGraphics::ShowHitboxes = !HitboxGraphics::ShowHitboxes;
    AddLog("  Hitboxes: %s", HitboxGraphics::ShowHitboxes ? "ON" : "OFF");
...
void Console::Render(Camera *camera, RenderContext &ctx) {
  ...
  ImGui::PushStyleColor(ImGuiCol_ScrollbarBg,
                        ImVec4(0.05f, 0.05f, 0.08f, 0.8f));
  ImGui::PushStyleColor(ImGuiCol_ScrollbarGrab,
                        ImVec4(0.25f, 0.60f, 0.30f, 0.8f));
  ...
  ImGui::TextColored(ImVec4(0.6f, 0.6f, 0.6f, 1.0f), "%.1f FPS | ~ to close",
                     io.Framerate);
  ImGui::Separator();
  ...
  if (m_ActivePanel != SettingsPanel::None) {
    ...
    ImGui::PushStyleColor(ImGuiCol_WindowBg,
                          ImVec4(0.06f, 0.06f, 0.10f, 0.95f));
...
void Console::RenderRenderingSettings(RenderContext &ctx) {
  ...
  if (ctx.msaaSamples == 2)
    currentMsaaIndex = 1;
  else if (ctx.msaaSamples == 4)
    currentMsaaIndex = 2;
  else if (ctx.msaaSamples == 8)
    currentMsaaIndex = 3;

  if (ImGui::Combo("MSAA##combo", &currentMsaaIndex, msaaOptions,
...
void Console::RenderPhysicsSettings() {
  ...
  ImGui::Separator();
  ImGui::DragFloat("Air Resistance", &PhysicsEngine::GlobalAirResistance, 0.01f,
                   0.0f, 10.0f);
...
void Console::RenderEnvironmentSettings(RenderContext &ctx) {
  ...
  float sunCol[4] = {ctx.sunColor.r, ctx.sunColor.g, ctx.sunColor.b,
                     ctx.sunColor.a};
  ...
  float moonCol[4] = {ctx.moonColor.r, ctx.moonColor.g, ctx.moonColor.b,
                      ctx.moonColor.a};
  ...
  if (m_ShowClouds) {
    ...
    ImGui::Combo("Cloud Mode", &ctx.cloudMode, cloudModes,
    ...
    ImGui::TextColored(ImVec4(0.5f, 0.5f, 0.5f, 1.0f),
                       "Clouds disabled. Enable via /clouds on");
  }
...
void Console::RenderLoggingScreen() {
  ...
  ImGui::TextColored(ImVec4(0.45f, 0.90f, 0.50f, 1.0f),
  ...
  ImGui::SameLine();
  ImGui::TextColored(ImVec4(0.9f, 0.6f, 0.2f, 1.0f), "%d",
                     StateManager::GetState());

  ImGui::Spacing();
  ImGui::Separator();
  ImGui::TextColored(ImVec4(0.45f, 0.90f, 0.50f, 1.0f),
                     "Sub-State Registrations");

  for (const auto &mapping : StateManager::GetAllStates()) {
    ImGui::Text("ID [%d] - ", mapping.first);
    ImGui::SameLine();
    ImGui::TextColored(ImVec4(0.6f, 0.8f, 0.9f, 1.0f), "%s",
