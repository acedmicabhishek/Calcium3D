...
bool EditorApplication::Init() {
  ...
  if (std::filesystem::exists("project.json")) {
    Logger::AddLog(
        "Standalone project config found! Booting into Game Mode...");
    try {
      ...
      if (config.contains("start_scene")) {
        std::string scenePath = m_ProjectRoot + "/Scenes/" +
                                config["start_scene"].get<std::string>();
...
void EditorApplication::LoadGlobalSettings() {
  ...
  try {
    ...
    if (settings.contains("dark_theme")) {
      ...
      if (m_EditorLayer->darkTheme)
        m_EditorLayer->ApplyDarkTheme();
      else
        m_EditorLayer->ApplyLightTheme();
    }
...
void EditorApplication::SaveProject(bool silent) {
  ...
  if (!currentScenePath.empty()) {
    if (currentScenePath.filename().string() ==
        "calcium3d_playmode_backup.scene") {

      config["last_scene"] = "Scenes/main.scene";
    } else {
      std::filesystem::path projRoot(m_ProjectRoot);
      std::string relPath =
          std::filesystem::relative(currentScenePath, projRoot).string();
  ...
  config["camera"]["position"] = {m_Camera->Position.x, m_Camera->Position.y,
                                  m_Camera->Position.z};
  ...
  config["graphics"]["msaaTransparencyPass"] =
      m_EditorLayer->msaaTransparencyPass;
  config["graphics"]["reflectionMode"] = m_EditorLayer->reflectionMode;
  config["graphics"]["ssrUseCubemapFallback"] =
      m_EditorLayer->ssrUseCubemapFallback;
  ...
  config["environment"]["cloudColor"] = {m_EditorLayer->cloudColor.x,
                                         m_EditorLayer->cloudColor.y,
                                         m_EditorLayer->cloudColor.z};
  ...
  config["environment"]["waterColor"] = {m_EditorLayer->waterColor.x,
                                         m_EditorLayer->waterColor.y,
                                         m_EditorLayer->waterColor.z};
  ...
  config["environment"]["sunColor"] = {
      m_EditorLayer->sunColor.x, m_EditorLayer->sunColor.y,
      m_EditorLayer->sunColor.z, m_EditorLayer->sunColor.w};
  ...
  config["environment"]["moonColor"] = {
      m_EditorLayer->moonColor.x, m_EditorLayer->moonColor.y,
      m_EditorLayer->moonColor.z, m_EditorLayer->moonColor.w};
  config["environment"]["moonBloom"] = m_EditorLayer->moonBloom;

  config["environment"]["enableShadows"] = m_EditorLayer->enableShadows;
  config["environment"]["enablePointShadows"] =
      m_EditorLayer->enablePointShadows;
...
void EditorApplication::OpenProject(const std::string &path) {
  ...
  if (std::filesystem::exists(scriptsDir)) {
    Logger::AddLog("[ScriptCompiler] Auto-compiling scripts in: %s",
                   scriptsDir.c_str());
  ...
  if (std::filesystem::exists(configPath)) {
    try {
      ...
      if (config.contains("last_scene")) {
        ...
        if (!lastScene.empty()) {
          std::filesystem::path fullScenePath =
              std::filesystem::path(path) / lastScene;
      ...
      if (config.contains("start_state")) {
        Application::Get().m_StartGameState =
            (GameState)config["start_state"].get<int>();
      }

      if (config.contains("camera")) {
        auto &cam = config["camera"];
        if (cam.contains("position")) {
          m_Camera->Position = glm::vec3(cam["position"][0], cam["position"][1],
                                         cam["position"][2]);
        }
        if (cam.contains("yaw"))
          m_Camera->yaw = cam["yaw"].get<float>();
        if (cam.contains("pitch"))
          m_Camera->pitch = cam["pitch"].get<float>();
      }

      if (config.contains("ui")) {
        auto &ui = config["ui"];
        if (ui.contains("showSceneHierarchy"))
          m_EditorLayer->showSceneHierarchy =
              ui["showSceneHierarchy"].get<bool>();
        if (ui.contains("showInspector"))
          m_EditorLayer->showInspector = ui["showInspector"].get<bool>();
        if (ui.contains("showConsole"))
          m_EditorLayer->showConsole = ui["showConsole"].get<bool>();
        if (ui.contains("showContentBrowser"))
          m_EditorLayer->showContentBrowser =
              ui["showContentBrowser"].get<bool>();
      }

      if (config.contains("graphics")) {
        auto &g = config["graphics"];
        if (g.contains("msaaSamples"))
          m_EditorLayer->msaaSamples = g["msaaSamples"].get<int>();
        if (g.contains("msaaSkyPass"))
          m_EditorLayer->msaaSkyPass = g["msaaSkyPass"].get<bool>();
        if (g.contains("msaaGeometryPass"))
          m_EditorLayer->msaaGeometryPass = g["msaaGeometryPass"].get<bool>();
        if (g.contains("msaaTransparencyPass"))
          m_EditorLayer->msaaTransparencyPass =
              g["msaaTransparencyPass"].get<bool>();
        if (g.contains("reflectionMode"))
          m_EditorLayer->reflectionMode = g["reflectionMode"].get<int>();
        if (g.contains("ssrUseCubemapFallback"))
          m_EditorLayer->ssrUseCubemapFallback =
              g["ssrUseCubemapFallback"].get<bool>();
        if (g.contains("ssrGeometry"))
          m_EditorLayer->ssrGeometry = g["ssrGeometry"].get<bool>();
        if (g.contains("ssrTransparency"))
          m_EditorLayer->ssrTransparency = g["ssrTransparency"].get<bool>();
        if (g.contains("ssrAll"))
          m_EditorLayer->ssrAll = g["ssrAll"].get<bool>();
        if (g.contains("ssrResolution"))
          m_EditorLayer->ssrResolution = g["ssrResolution"].get<float>();
        if (g.contains("ssrMaxSteps"))
          m_EditorLayer->ssrMaxSteps = g["ssrMaxSteps"].get<int>();
        if (g.contains("ssrMaxDistance"))
          m_EditorLayer->ssrMaxDistance = g["ssrMaxDistance"].get<float>();
        if (g.contains("ssrThickness"))
          m_EditorLayer->ssrThickness = g["ssrThickness"].get<float>();
        if (g.contains("ssrRenderDistance"))
          m_EditorLayer->ssrRenderDistance =
              g["ssrRenderDistance"].get<float>();
        if (g.contains("ssrFadeStart"))
          m_EditorLayer->ssrFadeStart = g["ssrFadeStart"].get<float>();
      }

      if (config.contains("environment")) {
        auto &env = config["environment"];
        if (env.contains("showSkybox"))
          m_EditorLayer->showSkybox = env["showSkybox"].get<bool>();
        if (env.contains("showGradientSky"))
          m_EditorLayer->showGradientSky = env["showGradientSky"].get<bool>();
        if (env.contains("showWater"))
          m_EditorLayer->showWater = env["showWater"].get<bool>();
        if (env.contains("showClouds"))
          m_EditorLayer->showClouds = env["showClouds"].get<bool>();
        if (env.contains("cloudMode"))
          m_EditorLayer->cloudMode = env["cloudMode"].get<int>();
        if (env.contains("waterHeight"))
          m_EditorLayer->waterHeight = env["waterHeight"].get<float>();
        if (env.contains("cloudHeight"))
          m_EditorLayer->cloud2dHeight = env["cloudHeight"].get<float>();
        if (env.contains("cloudDensity"))
          m_EditorLayer->cloudDensity = env["cloudDensity"].get<float>();
        if (env.contains("cloudCover"))
          m_EditorLayer->cloudCover = env["cloudCover"].get<float>();
        if (env.contains("cloudColor"))
          m_EditorLayer->cloudColor = glm::vec3(
              env["cloudColor"][0], env["cloudColor"][1], env["cloudColor"][2]);
        if (env.contains("cloudSpeed"))
          m_EditorLayer->cloudSpeed = env["cloudSpeed"].get<float>();
        if (env.contains("cloudTiling"))
          m_EditorLayer->cloudTiling = env["cloudTiling"].get<float>();
        if (env.contains("cloudSize"))
          m_EditorLayer->cloudSize = env["cloudSize"].get<float>();
        if (env.contains("cloudRandomness"))
          m_EditorLayer->cloudRandomness = env["cloudRandomness"].get<float>();

        if (env.contains("waveSpeed"))
          m_EditorLayer->waveSpeed = env["waveSpeed"].get<float>();
        if (env.contains("waveStrength"))
          m_EditorLayer->waveStrength = env["waveStrength"].get<float>();
        if (env.contains("waterColor"))
          m_EditorLayer->waterColor = glm::vec3(
              env["waterColor"][0], env["waterColor"][1], env["waterColor"][2]);
        if (env.contains("timeOfDay"))
          m_EditorLayer->timeOfDay = env["timeOfDay"].get<float>();

        if (env.contains("sunEnabled"))
          m_EditorLayer->sunEnabled = env["sunEnabled"].get<bool>();
        if (env.contains("sunIntensity"))
          m_EditorLayer->sunIntensity = env["sunIntensity"].get<float>();
        if (env.contains("sunColor"))
          m_EditorLayer->sunColor =
              glm::vec4(env["sunColor"][0], env["sunColor"][1],
                        env["sunColor"][2], env["sunColor"][3]);
        if (env.contains("sunBloom"))
          m_EditorLayer->sunBloom = env["sunBloom"].get<float>();

        if (env.contains("moonEnabled"))
          m_EditorLayer->moonEnabled = env["moonEnabled"].get<bool>();
        if (env.contains("moonIntensity"))
          m_EditorLayer->moonIntensity = env["moonIntensity"].get<float>();
        if (env.contains("moonColor"))
          m_EditorLayer->moonColor =
              glm::vec4(env["moonColor"][0], env["moonColor"][1],
                        env["moonColor"][2], env["moonColor"][3]);
        if (env.contains("moonBloom"))
          m_EditorLayer->moonBloom = env["moonBloom"].get<float>();

        if (env.contains("enableShadows"))
          m_EditorLayer->enableShadows = env["enableShadows"].get<bool>();
        if (env.contains("enablePointShadows"))
          m_EditorLayer->enablePointShadows =
              env["enablePointShadows"].get<bool>();
        if (env.contains("shadowBias"))
          m_EditorLayer->shadowBias = env["shadowBias"].get<float>();
      }
    } catch (const std::exception &e) {
...
void EditorApplication::ExitPlayMode() {
  ...
  if (!m_PlayModeSceneBackup.empty() &&
      std::filesystem::exists(m_PlayModeSceneBackup)) {
...
void EditorApplication::OnUpdate(float deltaTime) {
  ...
  if (!m_EditorLayer->isTimePaused) {
    ...
    if (m_EditorLayer->timeOfDay >= 24.0f)
      m_EditorLayer->timeOfDay -= 24.0f;
    if (m_EditorLayer->timeOfDay < 0.0f)
      m_EditorLayer->timeOfDay += 24.0f;
  }
...
void EditorApplication::OnRender() {
  ...
  m_EditorLayer->Begin();
  m_EditorLayer->UpdateViewportResolution(*m_Scene);

  if (m_EditorLayer->viewportWidth > 0 && m_EditorLayer->viewportHeight > 0 &&
      (m_EditorLayer->viewportWidth != m_ViewportWidth ||
       m_EditorLayer->viewportHeight != m_ViewportHeight)) {
    CreateViewportFramebuffer(m_EditorLayer->viewportWidth,
                              m_EditorLayer->viewportHeight);
...
void EditorApplication::RenderEditor(float deltaTime) {
  ...
  float angle =
      (m_EditorLayer->timeOfDay - 6.0f) / 24.0f * 2.0f * glm::pi<float>();
  glm::vec3 dynamicSunPos =
      glm::vec3(cos(angle) * 10.0f, sin(angle) * 10.0f, 0.0f);
  ...
  glm::vec3 dynamicMoonPos =
      glm::vec3(-dynamicSunPos.x, -dynamicSunPos.y, 0.0f);
