...
void RuntimeApplication::LoadProjectConfig() {
  ...
  if (std::filesystem::exists("project.json")) {
    ...
    try {
      ...
      if (config.contains("disable_state_warning"))
        m_ShowStateWarning = !config["disable_state_warning"];

      if (config.contains("ms_control"))
        m_Console->SetMasterControl(config["ms_control"]);
      if (config.contains("hitboxes"))
        m_Console->SetHitboxEnabled(config["hitboxes"]);
      ...
      if (std::filesystem::exists(uiLayoutPath)) {
        ...
        Logger::AddLog("[WARNING] No UI layout found at %s",
                       uiLayoutPath.c_str());
      }
      ...
      if (config.contains("environment")) {
        ...
        if (env.contains("showSkybox"))
          m_Console->SetSkyboxEnabled(env["showSkybox"]);
        if (env.contains("showGradientSky"))
          m_Console->SetGradientSkyEnabled(env["showGradientSky"]);
        if (env.contains("showClouds"))
          m_Console->SetCloudsEnabled(env["showClouds"]);

        if (env.contains("cloudMode"))
          m_RenderContext.cloudMode = env["cloudMode"];
        if (env.contains("cloudHeight"))
          m_RenderContext.cloudHeight = env["cloudHeight"];
        if (env.contains("cloudDensity"))
          m_RenderContext.cloudDensity = env["cloudDensity"];
        if (env.contains("cloudCover"))
          m_RenderContext.cloudCover = env["cloudCover"];
        if (env.contains("cloudSpeed") && m_Cloud2D)
          m_Cloud2D->cloudSpeed = env["cloudSpeed"];
        if (env.contains("cloudTiling") && m_Cloud2D)
          m_Cloud2D->tiling = env["cloudTiling"];
        if (env.contains("cloudSize") && m_Cloud2D)
          m_Cloud2D->cloudSize = env["cloudSize"];
        if (env.contains("cloudRandomness") && m_Cloud2D)
          m_Cloud2D->randomness = env["cloudRandomness"];
        ...
        if (env.contains("timeOfDay"))
          m_RenderContext.timeOfDay = env["timeOfDay"];

        if (env.contains("sunEnabled"))
          m_RenderContext.sunEnabled = env["sunEnabled"];
        if (env.contains("sunIntensity"))
          m_RenderContext.sunIntensity = env["sunIntensity"];
        ...
        if (env.contains("sunBloom"))
          m_RenderContext.sunBloom = env["sunBloom"];

        if (env.contains("moonEnabled"))
          m_RenderContext.moonEnabled = env["moonEnabled"];
        if (env.contains("moonIntensity"))
          m_RenderContext.moonIntensity = env["moonIntensity"];
        ...
        if (env.contains("moonBloom"))
          m_RenderContext.moonBloom = env["moonBloom"];

        if (env.contains("enableShadows"))
          m_RenderContext.enableShadows = env["enableShadows"];
        if (env.contains("enablePointShadows"))
          m_RenderContext.enablePointShadows = env["enablePointShadows"];
        if (env.contains("shadowBias"))
          m_RenderContext.shadowBias = env["shadowBias"];
      }

      if (config.contains("graphics")) {
        auto &gfx = config["graphics"];
        if (gfx.contains("reflectionMode"))
          m_RenderContext.reflectionMode = gfx["reflectionMode"];
        if (gfx.contains("ssrUseCubemapFallback"))
          m_RenderContext.ssrUseCubemapFallback = gfx["ssrUseCubemapFallback"];
        if (gfx.contains("ssrGeometry"))
          m_RenderContext.ssrGeometry = gfx["ssrGeometry"];
        if (gfx.contains("ssrTransparency"))
          m_RenderContext.ssrTransparency = gfx["ssrTransparency"];
        if (gfx.contains("ssrAll"))
          m_RenderContext.ssrAll = gfx["ssrAll"];
        if (gfx.contains("ssrResolution"))
          m_RenderContext.ssrResolution = gfx["ssrResolution"];
        if (gfx.contains("ssrMaxSteps"))
          m_RenderContext.ssrMaxSteps = gfx["ssrMaxSteps"];
        if (gfx.contains("ssrMaxDistance"))
          m_RenderContext.ssrMaxDistance = gfx["ssrMaxDistance"];
        if (gfx.contains("ssrThickness"))
          m_RenderContext.ssrThickness = gfx["ssrThickness"];
        if (gfx.contains("ssrRenderDistance"))
          m_RenderContext.ssrRenderDistance = gfx["ssrRenderDistance"];
        if (gfx.contains("ssrFadeStart"))
          m_RenderContext.ssrFadeStart = gfx["ssrFadeStart"];
      ...
      Logger::AddLog(
          "Loaded extracted Editor Environment & Graphics Settings.");
      ...
      if (config.contains("game_states")) {
        ...
        for (auto &item : jStates.items()) {
          ...
          if (name.find("Start Screen") != std::string::npos ||
              name.find("StartScreen") != std::string::npos ||
              name.find("Start Menu") != std::string::npos) {
            StateManager::RegisterState<StartScreen>(name);
          } else if (name.find("Gameplay") != std::string::npos ||
                     name.find("Main") != std::string::npos ||
                     name.find("Custom") != std::string::npos || id == 1) {
        ...
        Logger::AddLog("[State] Total registered state types: %d",
                       (int)registered.size());
        for (const auto &r : registered)
          Logger::AddLog("  - Native Logic Registered: '%s'", r.c_str());
      }
  ...
  if (!m_DisableStateWarning) {
    ...
    if (allStates.empty()) {
      ...
      Logger::AddLog(
          "[WARNING] No game states are registered! Showing fallback screen.");
    } else {
      bool startStateFound = false;
      for (auto const &[id, name] : allStates) {
        if (id == (int)m_StartGameState) {
      ...
      if (!startStateFound) {
        m_ShowStateWarning = true;
        Logger::AddLog("[WARNING] Start state ID %d not found in registered "
                       "states! Showing fallback screen.",
                       (int)m_StartGameState);
      }
    }

    if (!sceneLoaded || m_Scene->GetObjects().empty()) {
      m_ShowStateWarning = true;
      Logger::AddLog("[WARNING] Scene failed to load — states may be "
                     "misconfigured. Showing fallback screen.");
    }
    ...
    if (!std::filesystem::exists(uiCheck)) {
      m_ShowStateWarning = true;
      Logger::AddLog(
          "[WARNING] ui_layout.json not found — states may be misconfigured.");
    }
  }
...
void RuntimeApplication::CreateDefaultScene() {
  ...
  const glm::vec4 lightColors[] = {glm::vec4(1.0f, 0.3f, 0.2f, 1.0f),
  ...
  Logger::AddLog("Demo scene created: ground, 8 orbit cubes, 3 spheres, center "
                 "cube, 4 pillars, 3 lights");
}
...
void RuntimeApplication::OnUpdate(float deltaTime) {
  ...
  if (m_IsDemoScene && m_Window) {
    if (glfwGetKey(m_Window, GLFW_KEY_ESCAPE) == GLFW_PRESS &&
      ...
      m_CursorCaptured = false;
      if (m_Camera)
        m_Camera->m_cameraEnabled = false;
    }
  ...
  if ((m_IsDemoScene || m_Console->IsMasterControlEnabled()) && m_Camera &&
      m_Window && m_CursorCaptured) {
    float camSpeed = 8.0f * deltaTime;
    if (m_Console->IsMasterControlEnabled())
      camSpeed = 15.0f * deltaTime;
    ...
    float rotY =
        m_Camera->sensitivity * (float)(mx - centerX) / m_Camera->width;
    float rotX =
        m_Camera->sensitivity * (float)(my - centerY) / m_Camera->height;
    m_Camera->yaw += rotY;
    m_Camera->pitch -= rotX;
  ...
  if (m_IsDemoScene && m_Scene) {
    ...
    for (auto &obj : objects) {
      ...
      if (obj.name.rfind("OrbitCube_", 0) == 0) {
        ...
        obj.position.y =
            3.0f + idx * 3.0f + sin(m_DemoTime * 1.2f + idx) * 1.2f;
  ...
  if (m_Console && !m_Console->IsTimePaused()) {
    ...
    if (m_RenderContext.timeOfDay >= 24.0f)
      m_RenderContext.timeOfDay -= 24.0f;
    if (m_RenderContext.timeOfDay < 0.0f)
      m_RenderContext.timeOfDay += 24.0f;
  }
}
...
void RuntimeApplication::OnRender() {
  ...
  if (winW != m_ViewportWidth || winH != m_ViewportHeight) {
    ...
    if (m_MSAASamples > 0)
      ResizeMSAAFramebuffer(winW, winH);
  }
  ...
  float angle =
      (m_RenderContext.timeOfDay - 6.0f) / 24.0f * 2.0f * glm::pi<float>();
  m_RenderContext.sunPosition =
      glm::vec3(cos(angle) * 10.0f, sin(angle) * 10.0f, 0.0f);
  m_RenderContext.moonPosition = glm::vec3(
      -m_RenderContext.sunPosition.x, -m_RenderContext.sunPosition.y, 0.0f);
...

  {
    ...
    GPU_PROFILE_SCOPE("RenderPipeline");
  ...
  if (m_MSAASamples > 0) {
    ...
    glBlitFramebuffer(0, 0, winW, winH, 0, 0, winW, winH, GL_COLOR_BUFFER_BIT,
                      GL_NEAREST);
  }
...
void RuntimeApplication::PostRender() {
  ...
  if (m_ShowStateWarning) {
    ...
    float hintW = ImGui::CalcTextSize(
                      "Press F1 to toggle this warning | WASD to move camera")
                      .x;
  ...
  {
    ImGuiIO &io = ImGui::GetIO();
    ImGui::SetNextWindowPos(ImVec2(io.DisplaySize.x - 150, 8),
                            ImGuiCond_Always);
