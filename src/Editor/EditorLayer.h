...
public:
  ...
  void UpdateViewportResolution(Scene &scene);
  ...
  void SetContentPath(const std::string &path) {
    ...
    std::filesystem::path targetShadersDir =
        std::filesystem::path(path) / "Shaders";

    std::string sourceShaders = "";
    if (std::filesystem::exists("shaders"))
      sourceShaders = "shaders";
    else if (std::filesystem::exists("../shaders"))
      sourceShaders = "../shaders";
    else if (std::filesystem::exists("../../shaders"))
      sourceShaders = "../../shaders";
    ...
    if (!sourceShaders.empty()) {
      ...
      std::cerr << "[EditorLayer] Could not locate engine shaders directory to "
                   "copy!\n";
    }
  }
