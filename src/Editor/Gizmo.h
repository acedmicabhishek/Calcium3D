...
private:
  void setupGizmo();
  void createArrow(const glm::vec3 &direction, const glm::vec3 &color,
                   std::vector<float> &vertices);

  bool rayIntersectsAxis(const glm::vec3 &rayOrigin,
                         const glm::vec3 &rayDirection,
                         const glm::vec3 &axisStart, const glm::vec3 &axisEnd,
                         float axisRadius, float &t);

  void handleTranslate(GLFWwindow *window, Camera &camera,
                       const glm::vec3 &objectPosition, glm::vec3 &newPosition);
  void handleRotate(GLFWwindow *window, Camera &camera,
                    const glm::vec3 &objectPosition, glm::vec3 &newRotation);
  void handleScale(GLFWwindow *window, Camera &camera,
