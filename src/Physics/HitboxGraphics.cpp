...
void HitboxGraphics::Init() {
  ...
  unsigned int indices[] = {0, 1, 1, 3, 3, 2, 2, 0, 4, 5, 5, 7,
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * 24, nullptr,
               GL_DYNAMIC_DRAW);
...
void HitboxGraphics::Render(Scene &scene, Camera &camera) {
  ...
  for (auto &obj : scene.GetObjects()) {
    ...
    modelOBB = glm::translate(modelOBB, localCenter) *
    ...
    glm::mat4 modelAABB = glm::translate(glm::mat4(1.0f), aabbCenter) *
                          glm::scale(glm::mat4(1.0f), aabbSize);
    ...
    if (PhysicsEngine::GlobalCOMEnabled) {
      ...
      glm::mat4 modelCOM = glm::translate(glm::mat4(1.0f), worldCOM) *
  ...
  for (auto &obj : scene.GetObjects()) {
    ...
    if (obj.hasCamera && obj.camera.enabled) {
      ...
      glm::vec3 ntl(-nearW / 2, nearH / 2, -nearP),
          ntr(nearW / 2, nearH / 2, -nearP);
      glm::vec3 nbl(-nearW / 2, -nearH / 2, -nearP),
          nbr(nearW / 2, -nearH / 2, -nearP);
      glm::vec3 ftl(-farW / 2, farH / 2, -farP), ftr(farW / 2, farH / 2, -farP);
      glm::vec3 fbl(-farW / 2, -farH / 2, -farP),
          fbr(farW / 2, -farH / 2, -farP);

      std::vector<glm::vec3> lines = {ntl, ntr, ntr, nbr, nbr, nbl, nbl, ntl,
                                      ftl, ftr, ftr, fbr, fbr, fbl, fbl, ftl,
                                      ntl, ftl, ntr, ftr, nbl, fbl, nbr, fbr};

      glBindBuffer(GL_ARRAY_BUFFER, m_DynVBO);
      glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(glm::vec3) * lines.size(),
                      lines.data());
