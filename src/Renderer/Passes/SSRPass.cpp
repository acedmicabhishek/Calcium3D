...
void SSRPass::EnsureGeoNormalArchive(int w, int h) {
    ...
    if (s_GeometryNormalArchive != 0 && s_ArchiveWidth == w && s_ArchiveHeight == h) return;
    if (s_GeometryNormalArchive) { glDeleteTextures(1, &s_GeometryNormalArchive); s_GeometryNormalArchive = 0; }
    if (s_GeometryDepthArchive)  { glDeleteTextures(1, &s_GeometryDepthArchive);  s_GeometryDepthArchive = 0;  }
    if (s_ArchiveFBO)            { glDeleteFramebuffers(1, &s_ArchiveFBO);        s_ArchiveFBO = 0; }
    s_ArchiveWidth  = w;
    s_ArchiveHeight = h;
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, s_GeometryNormalArchive, 0);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_TEXTURE_2D, s_GeometryDepthArchive, 0);
...
void SSRPass::Init() {
    ...
    if (!ResourceManager::HasShader("ssr")) {
        ResourceManager::LoadShader("ssr", "../shaders/passes/postprocess/ssr.vert", "../shaders/passes/postprocess/ssr.frag");
    }
...
void SSRPass::ResizeResolveFBO(int width, int height) {
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_ResolveColorObj, 0);
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, m_ResolveNormalObj, 0);
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_TEXTURE_2D, m_ResolveDepthObj, 0);

    glBindFramebuffer(GL_FRAMEBUFFER, 0);
}

void SSRPass::Execute(const RenderContext& context) {
    if (context.width <= 0 || context.height <= 0 || context.mainFBO == 0) return;
    ...
    if (m_PassType == PassType::CubemapOnly) {
        ...
        if (!isSSR) return;     
        if (m_PassType == PassType::Geometry     && !context.ssrGeometry)     return;
        if (m_PassType == PassType::Transparency && !context.ssrTransparency) return;
        if (m_PassType == PassType::All          && !context.ssrAll)          return;
    }
    ...
    glBlitFramebuffer(0, 0, context.width, context.height, 0, 0, context.width, context.height, GL_DEPTH_BUFFER_BIT, GL_NEAREST);
    ...
    m_SSRShader->use();
    m_SSRShader->setMat4("projection", context.camera->GetProjectionMatrix());
    m_SSRShader->setMat4("view", context.camera->GetViewMatrix());
    m_SSRShader->setMat4("invProjection", glm::inverse(context.camera->GetProjectionMatrix()));
    m_SSRShader->setMat4("invView", glm::inverse(context.camera->GetViewMatrix()));
    ...
    m_SSRShader->setInt("ssrUseCubemapFallback", context.ssrUseCubemapFallback ? 1 : 0);
    m_SSRShader->setInt("reflectionMode", context.reflectionMode);
    
    m_SSRShader->setInt("isAllPass", (m_PassType == PassType::All) ? 1 : 0);

    
    m_SSRShader->setInt("u_showClouds", (context.showClouds && context.cloudMode == 0) ? 1 : 0);
