...
SkyPass::SkyPass() { m_Name = "SkyPass"; }
...
void SkyPass::Init() {
  ...
  if (s_SkyTexture == 0) {
    ...
    for (unsigned int i = 0; i < faces.size(); i++) {
      ...
      unsigned char *data =
          stbi_load(resolved.c_str(), &width, &height, &nrChannels, 0);
      if (data) {
...
void SkyPass::Execute(const RenderContext &context) {
  ...
  if (showGradient) {
    ...
    glm::vec3 bottomColor =
        glm::mix(nightBottomColor, dayBottomColor, dayNightCycle);

    glUniform3f(glGetUniformLocation(shader.ID, "topColor"), topColor.x,
                topColor.y, topColor.z);
    glUniform3f(glGetUniformLocation(shader.ID, "bottomColor"), bottomColor.x,
                bottomColor.y, bottomColor.z);
    ...
    glUniform3f(glGetUniformLocation(shader.ID, "DynamicSunPos"),
                DynamicSunPos.x, DynamicSunPos.y, DynamicSunPos.z);
    glUniform3f(glGetUniformLocation(shader.ID, "DynamicMoonPos"),
                DynamicMoonPos.x, DynamicMoonPos.y, DynamicMoonPos.z);

    glUniform1f(glGetUniformLocation(shader.ID, "sunBloom"), context.sunBloom);
    glUniform1f(glGetUniformLocation(shader.ID, "moonBloom"),
                context.moonBloom);
