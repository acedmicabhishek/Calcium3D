...
void UnderwaterPass::Init() {
    SetupQuad();
}

void UnderwaterPass::Execute(const RenderContext& context) {
    if (!context.scene || !context.camera || context.mainFBO == 0) return;
    ...
    for (int i = 0; i < (int)objects.size(); ++i) {
        ...
        if (obj.hasWater && obj.isActive) {
            ...
            glm::vec4 localCamPosVec = invModel * glm::vec4(context.camera->Position, 1.0f);
            ...
            float surfaceLocal = obj.water.surfaceHeight / (obj.scale.y > 0.001f ? obj.scale.y : 1.0f);
            float bottomLocal = (obj.water.surfaceHeight - obj.water.depth) / (obj.scale.y > 0.001f ? obj.scale.y : 1.0f);

            
            bool isInfinite = (obj.scale.x > 5000.0f || obj.scale.z > 5000.0f);

            if ((insideXZ || isInfinite) && localCamPos.y <= surfaceLocal && localCamPos.y >= bottomLocal) {
    ...
    if (!ResourceManager::HasShader("underwater")) {
        ResourceManager::LoadShader("underwater", "../shaders/passes/postprocess/underwater.vert", "../shaders/passes/postprocess/underwater.frag");
    }
    ...
    for (auto& obj : objects) {
        if (obj.hasWater && obj.isActive) {
            ...
            glm::mat4 invModel = glm::inverse(model);
            glm::vec3 localCam = glm::vec3(invModel * glm::vec4(context.camera->Position, 1.0f));
            ...
            bool isInfinite = (obj.scale.x > 5000.0f || obj.scale.z > 5000.0f);
            bool insideXZ = isInfinite || (localCam.x >= -0.52f && localCam.x <= 0.52f &&
                                          localCam.z >= -0.52f && localCam.z <= 0.52f);

            if (insideXZ && localCam.y <= surfaceL && localCam.y >= bottomL) {
                
                shader.setMat4("invModel", invModel);
                shader.setVec3("localBoxMin", glm::vec3(isInfinite ? -1e6f : -0.5f, bottomL, isInfinite ? -1e6f : -0.5f));
                shader.setVec3("localBoxMax", glm::vec3(isInfinite ? 1e6f : 0.5f, surfaceL, isInfinite ? 1e6f : 0.5f));
                shader.setFloat("liquidDensity", obj.water.liquidDensity);
                
                shader.setMat4("invView", glm::inverse(context.camera->GetViewMatrix()));
                shader.setMat4("invProjection", glm::inverse(context.camera->GetProjectionMatrix()));
