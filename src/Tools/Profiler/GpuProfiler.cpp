...
void GpuProfiler::SetEnabled(bool v) {
    if (v && !m_Initialized) InitQueries();
    m_Enabled = v;
    if (!v) m_FinishedSamples.clear();
}
...
void GpuProfiler::BeginFrame() {
    ...
    if (m_TotalFrames >= GPU_FRAME_LAG) {
        ...
        for (int i = 0; i < readFrame.count; i++) {
            ...
            if (!avail) { allReady = false; continue; }
...
void GpuProfiler::BeginSample(const char* name) {
    if (!m_Enabled || m_Paused || !m_Initialized) return;
    FrameBuffer& f = m_Frames[m_WriteIdx];
    if (f.count >= GPU_MAX_SAMPLES) return;
...
void GpuProfiler::EndSample(const char* name) {
    ...
    for (int i = f.count - 1; i >= 0; i--) {
        if (f.slots[i].used && strncmp(f.slots[i].name, name, sizeof(f.slots[i].name)) == 0) {
