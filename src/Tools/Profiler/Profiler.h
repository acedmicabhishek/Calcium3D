...
#define PROFILE_SCOPE(name)                                                    \
  PROFILE_CONCAT(ProfilerScope _prof_scope_, __LINE__)(name)
...
public:
  ...
  const std::array<ProfileFrame, PROFILER_HISTORY> &GetFrameHistory() const {
    return m_FrameHistory;
  }
  int GetCurrentFrameIdx() const { return m_FrameIdx; }
  const ProfileFrame &GetLastFrame() const {
    return m_FrameHistory[(m_FrameIdx - 1 + PROFILER_HISTORY) %
                          PROFILER_HISTORY];
  }
