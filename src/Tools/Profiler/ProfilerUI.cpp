...
struct CatDef { const char* label; ImVec4 color; };
...
static ImVec4 GetColor(const std::string& name) {
    for (auto& c : CATS) if (name == c.label) return c.color;
...
void ProfilerUI::Draw(bool* pOpen) {
    ...
    if (!ImGui::Begin("Profiler##C3D", pOpen)) {
        ImGui::PopStyleColor(2); ImGui::End(); return;
    }
    ...
    ImGui::PushStyleColor(ImGuiCol_Button,
        enabled ? ImVec4(0.1f,0.7f,0.3f,1.0f) : ImVec4(0.3f,0.3f,0.35f,1.0f));
    if (ImGui::Button(enabled ? "  ● REC  " : "  ○ REC  ")) prof.SetEnabled(!enabled);
    ImGui::PopStyleColor();
    ImGui::SameLine();
    bool paused = prof.IsPaused();
    if (ImGui::Button(paused ? "  ▶ Resume  " : "  ⏸ Pause  ")) prof.SetPaused(!paused);
    ImGui::SameLine();
    if (ImGui::Button("  ✖ Clear  ")) {
        for (auto& f : const_cast<std::array<ProfileFrame,PROFILER_HISTORY>&>(history)) f = {};
        s_SelFrame = -1;
    }

    const ProfileFrame& liveFrame = (s_SelFrame >= 0) ? history[s_SelFrame] : prof.GetLastFrame();
    ...
    ImGui::PopStyleColor();
    ImGui::SameLine(0,12); ImGui::TextDisabled("Avg %.0f FPS", avgFps);

    
    ImGui::SameLine(0,16);
    bool gpuHasData = !gpu.GetLastSamples().empty();
    ImGui::PushStyleColor(ImGuiCol_Text,
        gpuHasData ? ImVec4(0.3f,0.9f,0.4f,1) : ImVec4(0.6f,0.4f,0.2f,1));
    ImGui::Text(gpuHasData ? "  ● GPU Queries Active" : "  ○ GPU Queries (waiting...)");
    ImGui::PopStyleColor();
    ImGui::Separator();
    ...
    for (int i = 0; i < PROFILER_HISTORY; i++)
        if (history[i].totalMs > maxFrameMs) maxFrameMs = history[i].totalMs;
    ...
    draw->AddRectFilled(gPos, ImVec2(gPos.x+gW, gPos.y+graphH), IM_COL32(15,15,20,255), 4.0f);

    auto yForMs = [&](float t) { return gPos.y + graphH - (t/maxMs)*graphH; };
    for (float tgt : {16.7f, 33.3f}) {
        if (tgt >= maxMs) continue;
        float y = yForMs(tgt);
        draw->AddLine(ImVec2(gPos.x,y), ImVec2(gPos.x+gW,y), IM_COL32(200,200,80,50));
        char lb[12]; snprintf(lb,sizeof(lb),"%.0fms",tgt);
    ...
    for (int i = 0; i < PROFILER_HISTORY; i++) {
        ...
        draw->AddRectFilled(ImVec2(x0, gPos.y+graphH-totalH), ImVec2(x1, gPos.y+graphH),
        ...
        if (sel) draw->AddRect(ImVec2(x0,gPos.y), ImVec2(x1,gPos.y+graphH), IM_COL32(255,220,60,230));
    }
    ...
    if (ImGui::IsItemHovered()) {
        ...
        if (slot >= 0 && slot < PROFILER_HISTORY) {
            ...
            if (hf.totalMs > 0.f) {
                ...
                ImGui::EndTooltip();
                if (ImGui::IsMouseClicked(ImGuiMouseButton_Left)) s_SelFrame = hi;
            }
        }
    }
    ...
    const ProfileFrame& frame = (s_SelFrame >= 0) ? history[s_SelFrame] : prof.GetLastFrame();
    ...
    auto        gpuMs = [&](const std::string& name) -> float {
        for (auto& g : gpuSamples) if (g.name == name) return g.durationMs;
        return -1.f;
    };

    
    auto sorted = frame.samples;
    std::sort(sorted.begin(), sorted.end(),
        [](const ProfileSample& a, const ProfileSample& b){ return a.durationMs > b.durationMs; });

    ImGui::TextDisabled("Frame: %.2f ms  |  %.1f FPS   |   GPU data: 1-frame delayed (OpenGL async)",
        frameMs, frame.fps);
    ImGui::Spacing();

    ImGui::PushStyleColor(ImGuiCol_TableRowBg,    ImVec4(0.10f,0.10f,0.12f,1.0f));
    ImGui::PushStyleColor(ImGuiCol_TableRowBgAlt, ImVec4(0.12f,0.12f,0.15f,1.0f));
    ...
    if (ImGui::BeginTable("##ptable", 5,
        ...
        ImGui::TableSetupColumn("CPU Bar",  ImGuiTableColumnFlags_WidthStretch, 0.25f);
        ImGui::TableSetupColumn("GPU ms",   ImGuiTableColumnFlags_WidthFixed,   62.0f);
        ImGui::TableSetupColumn("GPU Bar",  ImGuiTableColumnFlags_WidthStretch, 0.30f);
        ...
        auto drawBar = [&](float ms, float maxForBar, ImU32 col, bool dimmed) {
            ...
            ImU32  col2 = dimmed
                ...
                : col;
            if (rW > 0) draw->AddRectFilled(rMin, ImVec2(rMin.x+rW, rMin.y+15.f), col2, 2.f);
            if (rW > 28) {
                char pct[10]; snprintf(pct,sizeof(pct),"%.1f%%", ratio*100.f);
                draw->AddText(ImVec2(rMin.x+4,rMin.y+1), IM_COL32(255,255,255,180), pct);
            }
        ...
        float maxGpuMs = 1.0f;
        for (auto& g : gpuSamples) if (g.durationMs > maxGpuMs) maxGpuMs = g.durationMs;
        ...
        for (auto& g : gpuSamples) {
            bool found = false;
            for (auto& s : sorted) if (s.name == g.name) { found = true; break; }
            if (found) continue;
            ...
            ImGui::TableSetColumnIndex(2); ImGui::Dummy(ImVec2(0,16));
            ImGui::TableSetColumnIndex(3); ImGui::Text("%.3f", g.durationMs);
            ImGui::TableSetColumnIndex(4); drawBar(g.durationMs, maxGpuMs, u32, false);
        }
    ...
    float cpuTotal = 0.f; for (auto& s : frame.samples) cpuTotal += s.durationMs;
    float gpuTotal = 0.f; for (auto& g : gpuSamples) gpuTotal += g.durationMs;
    ImGui::TextDisabled(
        "CPU sampled: %.2f ms  |  GPU sampled: %.2f ms  |  Frame total: %.2f ms  [GL_TIMESTAMP queries]",
